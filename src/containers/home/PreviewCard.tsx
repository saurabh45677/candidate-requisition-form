...
const PreviewCard: React.FC<{
  ...
  state?: any;
}> = ({ requisitionDetails, jobDetails, interviewSettings, state }) => {
  //function to make the opening value 0 if the  field is empty
  const getNoOfOpenings = (value: number) => {
    if (!value || 0) return 0;
    else return value;
  };
      ...
      <Box borderRadius="10px" bgColor="gray.100" height="fit-content">
            ...
            >
              ...
              <Flex justifyContent="space-around" alignItems="center">
                ...
                <Text fontSize="1rem" fontWeight="bold" as="span">
                  {getNoOfOpenings(state?.requisitionDetails?.noOfOpenings)}
                </Text>
              </Flex>
            </Flex>
          </Box>
        ...
        <Box maxH="50rem" overflowY="auto" px="24px" pb="24px">
          <DataCard title="Requisition Details">
            <KeyValue
              title="Urgency"
              value={state?.requisitionDetails?.urgency}
            />
            <KeyValue
              title="Gender"
